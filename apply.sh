...
original_author="onipaa"
original_name="cicd_example_01"
original_urlname="cicd-example-01"
original_description="Awesome cicd_example_01 created by onipaa" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/cicd_example_01 "${name}"
...
then
    ...
    echo "then run 'cicd_example_01' or 'python -m cicd_example_01'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'cicd_example_01' or 'python -m cicd_example_01' or 'poetry run cicd_example_01'"
fi
